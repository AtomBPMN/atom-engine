...
func (itee *IntermediateThrowEventExecutor) Execute(token *models.Token, element map[string]interface{}) (*ExecutionResult, error) {
...
func (itee *IntermediateThrowEventExecutor) handleMessageThrowEvent(token *models.Token, element map[string]interface{}, eventDef map[string]interface{}) (*ExecutionResult, error) {
...
func (itee *IntermediateThrowEventExecutor) handleSignalThrowEvent(token *models.Token, element map[string]interface{}, eventDef map[string]interface{}) (*ExecutionResult, error) {
...
func (itee *IntermediateThrowEventExecutor) executeRegularThrowEvent(token *models.Token, element map[string]interface{}) (*ExecutionResult, error) {
	// Get outgoing sequence flows
	outgoing, exists := element["outgoing"]
	if !exists {
		// Throw events without outgoing flows complete the token
...
func (itee *IntermediateThrowEventExecutor) extractCorrelationKeyFromMessage(token *models.Token, messageID string) string {
	// Get full BPMN process definition
	// Получаем полное определение BPMN процесса
	bpmnProcess, err := itee.processComponent.GetBPMNProcessForToken(token)
	if err != nil {
