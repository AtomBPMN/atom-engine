...
func (pim *ProcessInstanceManager) StartProcessInstance(
	processKey string,
...
func (pim *ProcessInstanceManager) CancelProcessInstance(instanceID string, reason string) (err error) {
	...
	// Cancel all jobs for this process instance
	// Отменяем все job для этого экземпляра процесса
	if err := pim.component.CancelAllJobsForProcessInstance(instanceID, reason); err != nil {
		logger.Error("Failed to cancel all process jobs",
			logger.String("instance_id", instanceID),
			logger.String("error", err.Error()))
		// Continue even if job cancellation fails
...
func (pim *ProcessInstanceManager) ListProcessInstances(
	statusFilter string,
