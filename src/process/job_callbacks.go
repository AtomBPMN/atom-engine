...
func extractErrorCodeFromMessage(errorMessage string) string {
	...
	patterns := []struct {
		...
		{`(?i)(timeout|connection[_\s]*failed|not[_\s]*found|unauthorized|forbidden|internal[_\s]*error)`, 1},
	}
...
func (jc *JobCallbacks) activateErrorBoundaryFlow(
...
func (jc *JobCallbacks) handleJobBPMNError(
	token *models.Token,
	jobID, elementID, errorMessage string,
	variables map[string]interface{},
) error {
...
func (jc *JobCallbacks) createUnhandledBPMNErrorIncident(
	token *models.Token,
	elementID, errorCode, errorMessage string,
) error {
	if jc.component == nil {
		return fmt.Errorf("component not available")
	}
