...
func (s *BadgerStorage) applyPerformanceOptions(opts *badger.Options) {
	...
	if s.config.Options.Performance != nil {
		...
		if perf.MaxTableSize != nil {
			logger.Debug("MaxTableSize not supported in BadgerDB v3, consider using BaseTableSize",
				logger.Int64("value", *perf.MaxTableSize),
			)
		}
		...
		if perf.TableLoadingMode != nil {
			logger.Debug("TableLoadingMode setting noted (implementation depends on BadgerDB version)",
				logger.String("mode", *perf.TableLoadingMode),
			)
			// NOTE: Field name may vary in different BadgerDB versions
		}

		if perf.ValueLogLoadingMode != nil {
			logger.Debug("ValueLogLoadingMode setting noted (implementation depends on BadgerDB version)",
				logger.String("mode", *perf.ValueLogLoadingMode),
			)
			// NOTE: Field name may vary in different BadgerDB versions
		}
