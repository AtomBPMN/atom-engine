...
func (bm *BufferManager) ListBufferedMessages(
	ctx context.Context,
	tenantID string,
	limit, offset int,
...
func (bm *BufferManager) ProcessBufferedMessages(
	...
	if processedCount > 0 {
		bm.logger.Info("Processed buffered messages",
			logger.String("subscriptionID", subscription.ID),
			logger.Int("processedCount", processedCount),
		)
	}
...
func (bm *BufferManager) GetBufferedMessagesByName(
	...
	bm.logger.Debug("Found buffered messages by name",
...
func (bm *BufferManager) GetBufferedMessagesByCorrelationKey(
	ctx context.Context,
	tenantID, correlationKey string,
) ([]*models.BufferedMessage, error) {
	...
	bm.logger.Debug("Found buffered messages by correlation key",
