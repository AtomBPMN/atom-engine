...
func NewCorrelationManager(storage storage.Storage, logger logger.ComponentLogger, responseChannel chan string) *CorrelationManager {
...
func (cm *CorrelationManager) PublishMessage(ctx context.Context, tenantID, messageName, correlationKey, elementID string, variables map[string]interface{}, ttl *time.Duration) (*models.MessageCorrelationResult, error) {
	...
	if targetSubscription != nil {
		...
		if isIntermediateCatchEvent {
			...
			cm.logger.Info("Message correlated, process instance created", logger.String("processInstanceID", processInstanceID), logger.String("subscriptionID", targetSubscription.ID))
		}
		...
		if cm.responseChannel != nil {
			...
			if isIntermediateCatchEvent {
				if waitingToken, err := cm.findWaitingToken(targetSubscription.StartEventID, messageName); err == nil && waitingToken != nil {
...
func (cm *CorrelationManager) GetStats(ctx context.Context, tenantID string) (*MessageStats, error) {
	...
	cm.logger.Debug("Message stats calculated", logger.Int("bufferedMessages", stats.BufferedMessages), logger.Int("publishedToday", stats.PublishedToday))
