...
func (sm *SubscriptionManager) CreateSubscription(ctx context.Context, subscription *models.ProcessMessageSubscription) error {
	sm.logger.Info("Creating message subscription", logger.String("messageName", subscription.MessageName), logger.String("startEventID", subscription.StartEventID))

	// Check if subscription already exists
	existing, err := sm.storage.GetProcessMessageSubscription(ctx, subscription.TenantID, subscription.ProcessDefinitionKey, subscription.StartEventID)
	if err != nil {
	...
	if existing != nil {
		...
		return fmt.Errorf("subscription already exists for process %s, event %s message_name=%s", subscription.ProcessDefinitionKey, subscription.StartEventID, subscription.MessageName)
	}
...
func (sm *SubscriptionManager) DeleteSubscription(ctx context.Context, subscriptionID string) error {
	...
	sm.logger.Info("Message subscription deleted", logger.String("processKey", subscription.ProcessDefinitionKey), logger.String("messageName", subscription.MessageName))

	return nil
}

// ListSubscriptions lists message subscriptions
func (sm *SubscriptionManager) ListSubscriptions(ctx context.Context, tenantID string, limit, offset int) ([]*models.ProcessMessageSubscription, error) {
...
func (sm *SubscriptionManager) GetSubscription(ctx context.Context, tenantID, processKey, startEventID string) (*models.ProcessMessageSubscription, error) {
	sm.logger.Debug("Getting message subscription", logger.String("processKey", processKey), logger.String("startEventID", startEventID))
...
func (sm *SubscriptionManager) GetSubscriptionByID(ctx context.Context, subscriptionID string) (*models.ProcessMessageSubscription, error) {
...
func (sm *SubscriptionManager) UpdateSubscription(ctx context.Context, subscription *models.ProcessMessageSubscription) error {
...
func (sm *SubscriptionManager) GetSubscriptionsByMessageName(ctx context.Context, tenantID, messageName string) ([]*models.ProcessMessageSubscription, error) {
	...
	sm.logger.Debug("Found subscriptions by message name", logger.String("messageName", messageName), logger.Int("count", len(subscriptions)))
