...
func (p *ProcessParser) Parse(element *XMLElement, context *ParseContext) (map[string]interface{}, error) {
	...
	for _, child := range element.Children {
		...
		case "startEvent", "endEvent", "intermediateCatchEvent", "intermediateThrowEvent", "boundaryEvent":
			flowElements["events"]++
		case "task", "userTask", "serviceTask", "scriptTask", "sendTask", "receiveTask", "manualTask", "businessRuleTask", "callActivity", "subProcess":
...
func (p *ProcessParser) parseExtensionElements(element *XMLElement) map[string]interface{} {
	result := make(map[string]interface{})
...
func (p *ProcessParser) parseZeebeProperties(element *XMLElement) []map[string]string {
...
func (p *ProcessParser) parseZeebeTaskDefinition(element *XMLElement) map[string]string {
