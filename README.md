...
The project uses golangci-lint for code quality checks. Configuration is local-only and not committed to git.
...
# Or install directly using one of these methods:
...
# The first run will automatically create .golangci.yml from .golangci.yml.example
# You can customize .golangci.yml locally (it's ignored by git)
```

**Configuration:**

- `.golangci.yml.example` - Example configuration (committed to git)
- `.golangci.yml` - Local configuration (ignored by git, auto-created from example)

The lint configuration enforces:
- Maximum cyclomatic complexity: 15
- Code quality rules (gocritic, govet, gosec)
- Import and formatting standards (goimports, gofmt)
